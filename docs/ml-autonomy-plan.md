...
- **Текущее доказанное состояние:** Фаза 0 завершена 6 сентября 2026 года на
  ...
  точка отсчёта, а не promotion модели. Обучение и следующие фазы не начаты;
  для read-only аудита Фазы 1 нужна отдельно разрешённая владельцем цель.
...
Количество, 1080p/MP4 и размеры пока известны со слов владельца; файлы ещё не
проверены и не считаются восемью независимыми разрешёнными источниками автоматически.
...
### Следующий инкремент требует отдельной цели

Фаза 0 завершена; её принятый baseline и неизменённые отрицательные контроли
сохраняются как исходная точка. Текущая цель не разрешает начинать Фазу 1,
загружать X-CLIP или QLoRA, импортировать данные либо обучать модель.

Следующая предлагаемая ограниченная цель — первый срез Фазы 1: read-only аудит
данных и provenance восьми заявленных файлов UBA в `Данные матчи/`. NBA отложена
по решению владельца от 2026-09-11. Уточнение очередности не запускает аудит или
обучение.
После отдельного разрешения владельца этот срез должен:
...
Результат среза — inventory, source/rights ledger и coverage/blocker report с
планом разметки и разбиения. Это ещё не sealed dataset и не доказательство качества.
Сам аудит не разрешает импорт, выгрузку, изменение пользовательских данных или
обучение; допустимые источники и границы доступа должны входить в новую цель.
