...
- **Текущее доказанное состояние:** Фаза 0 открыта. Чистый `7054f13` на M4 Pro
  ...
  Пять benchmark-профилей и десять прямых verifier cases (3 совпадения, 7 модельных
  промахов, 0 инфраструктурных ошибок) относятся только к
  [прежнему `1b11a54`](benchmarks/phase0/negative-controls/1b11a54/README.md);
  их нельзя объединять с новым SHA. Новый batch для `7054f13` и принятый baseline
  bundle отсутствуют; входы для одного SHA готовятся независимо. Следующий
  ограниченный эксперимент — неизменённый обычный
  full smoke после предоставленной владельцем свежей macOS-сессии. На 6 сентября
  система всё ещё загружена 18 августа; свежая сессия не гарантирует нулевой swap.
