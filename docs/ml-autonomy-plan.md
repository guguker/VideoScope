...
### Правило работы владельца — 2026-09-11

Во всём проекте основным процессом выбран **Superpowers**, начиная с
`superpowers:using-superpowers`; остальные skills дополняют его предметными
проверками. Прямые указания владельца и платформенные инструкции имеют приоритет
над skills. Архитектурные, data и ML-инварианты этого плана сохраняются.

Единица поставки — **крупный законченный пользовательский сценарий**. Агент заранее
продумывает, как человек выполнит всю работу, и доводит уже разрешённый сценарий
до пригодного инструмента, включая сохранение, продолжение, исправление и проверку
результата. Не дробить работу на искусственные остановки ради отдельных кнопок,
полей или малых подборок. Технические подзадачи, тесты и коммиты могут оставаться
небольшими; их границы не требуют повторного согласования обычных действий.

Скрупулёзность не уменьшается: contracts, воспроизводимость, проверки состояния,
релевантные полные тесты и rollback обязательны по характеру изменения. Правило
одной гипотезы и одного principal change относится к измеряемому ML-эксперименту;
связанный продуктовый результат может требовать нескольких компонентов. Новые
способы использования данных, обучение, promotion и реальные phase gates не
становятся разрешёнными из-за увеличения шага. Перечитывание, повторные проверки
и делегирование должны закрывать конкретную неопределённость, а не создавать
...
- **Сделать:** реализовать source/rights ledger, annotation format,
  ...
  аудит внешних datasets; дать удобный локальный инструмент для последовательной
  разметки полных матчей, correction и adjudication с сохранением прогресса;
...
### Выполненный первый срез Фазы 1 — подборка для разметки
...
### Текущая цель Фазы 1 — полноценное локальное рабочее место разметки

Уточнение владельца 2026-09-11: он готов вручную разметить все восемь матчей UBA,
если инструмент удобен. Поэтому 24 превью и предложенная проба на 5–10 владениях
не являются пределом работы. Они могут служить проверкой интерфейса и качества
разметки; цель инструмента — длительная последовательная работа с полными матчами.

> Довести локальную разметку до законченного пользовательского сценария:
> открыть разрешённый матч, продолжить с сохранённого места, выделить владения и
> несколько событий, указать участников и наблюдаемые позиции, исправить ответы
> без их повторного ввода и получить проверяемый экспорт с историей и происхождением.

Обновление 2026-09-18: реализация рабочего места теперь включена в ветку
`codex/annotation-workbench`. Локальная проверка публикационной копии прошла
backend/frontend suites, синтетические browser-сценарии и backup/restore.
[Статус проверки](benchmarks/phase1/workbench/README.md) явно отмечает dirty checkout,
оставшиеся ограничения локальных черновиков и отсутствие clean-release receipt.
Это проверка инструмента; sealed dataset и полный data exit gate ещё не готовы.

Ниже сохранён контракт поставки, по которому проверяется реализация.
Связанные изменения интерфейса, хранения, data contract и проверок выполняются
как единая работа. Не перекладывать на владельца ручную нарезку или необходимость
заказывать каждую очевидную недостающую кнопку отдельно.

**Контракт результата:**

1. Навигация по целому матчу и списку событий/владений, точный переход по времени,
   удобные границы, несколько событий в одном контексте, продолжение работы после
   закрытия вкладки и перезапуска. Ясно различать сохранённое и черновик.
2. Участники связаны с командой внутри матча; цвет формы и видимый номер —
   наблюдения. Последняя передача, её участники и время хранятся отдельно от
   утверждения об официальном assist. Невидимое и нечитаемое не угадываются.
3. Пространственная разметка выполняется на выбранных кадрах с привязкой ко
   времени, игроку/команде и видимости. Различать координаты изображения и
   калиброванные координаты площадки; поддержать будущие траектории, не объявляя
   автоматический tracking, геометрию или распознавание защиты уже доказанными.
4. Существующие события и неизменяемые ревизии сохраняются; новые дополнительные
   сведения дописываются к ним. Добавление optional-полей не требует заново
   отвечать на старые вопросы. Экспорт, восстановление и совместимость версий
   проверяются на синтетических fixtures без записи тестовых меток владельцу.
5. Сохранить source/rights ledger, game/replay groups, annotation policy,
   coverage report и deterministic validation/split/seal. Не превращать
   человеческий ответ в gold без соответствующей политики и adjudication.
   Технически пригодный инструмент и готовый sealed dataset — разные результаты;
   отсутствие достаточного gold не оправдывает незаконченный интерфейс.

**Проверка готовности инструмента:** contract/backend/frontend tests, релевантные
полные suites и браузерный сценарий на синтетическом материале: открыть длинную
запись, создать несколько событий и владений с участниками/позициями, сохранить,
закрыть и перезапустить, продолжить, исправить, экспортировать и восстановить.
Проверить конфликт сохранения, миграцию старых ревизий, отсутствие внешних запросов
и потерянных ответов. Затем выполнить разрешённую локальную проверку удобства на
целевом Mac. Команды и receipts записываются с фактическими результатами;
пока нового прогона нет, этот контракт остаётся непроверенным.

Текущие роли двух `development_review`, пяти `reserve_uninspected` и одного
подозреваемого regression source сохраняются до явного изменения source ledger.
Способность инструмента открыть восемь матчей не означает автоматический просмотр
резерва. При расширении человеческой разметки заранее записать роли и правила
разделения; материал, просмотренный для разработки и разбора ошибок, не выдавать
за untouched holdout. Само желание разметить восемь матчей не разрешает training,
публикацию или внешнюю передачу данных.

Аудит публичных описаний готовых datasets и checkpoints, включая защитные
взаимодействия, ведётся как часть выбора источников: отдельно установить наличие
разметки, весов, доступность и права. Совместимость чужого checkpoint с M4 Pro и
качество на UBA требуют собственных измерений. Следующий исследовательский шаг
выбирается по покрытию задачи, а не по размеру модели.
Первый [обзор кандидатов от 2026-09-11](benchmarks/phase1/external-candidates-2026-09-11.md)
выделяет BARD/E-BARD для событий/участников, SportsHHI для взаимодействий,
SportsMOT/MixSort и DeepSportradar для tracking/geometry. Это результаты проверки
публичных описаний; внешний dataset или checkpoint ещё не принят.

Обучение остаётся закрыто до полного data exit gate и frozen candidate-recall
prerequisites. Первый learner после их прохождения — frozen SigLIP2 feature
control согласно Фазе 2. Baseline и rollback Фазы 0 не изменяются.
