...
- **Текущее доказанное состояние:** Фаза 0 открыта. Чистый `1b11a54` на M4 Pro
  прошёл 2 441 backend-тест, аттестацию шести окружений и принудительный rollback.
  Выполнены пять продуктовых профилей и десять прямых verifier cases:
  3 совпадения, 7 модельных промахов, 0 инфраструктурных ошибок. Полный smoke завершил
  функциональный путь, но collector отклонил его из-за 446 страниц swap-in.
  После подтверждения владельцем спокойного окна Mac единственный неизменённый
  повтор также отклонён: 116 страниц swap-in, 0 swap-out и Metal recovery;
  cleanup завершён. Оба отказа сохранены, принятый baseline bundle пока отсутствует.
  Добавлена опциональная [диагностическая шкала](benchmarks/phase0/diagnostic-timeline.md)
  этапов и RSS по ролям worker, без изменения жизненного цикла моделей или smoke
  coverage. Следующий контроль измерит её из нового чистого checkout.
  Принадлежность swap-in конкретному процессу пока не установлена.
  Точные receipts и ограничения
  качества сохранены в [контрольном отчёте](benchmarks/phase0/negative-controls/1b11a54/README.md).
  Нулевой swap gate сохраняется; обучение и следующие фазы не начаты.
