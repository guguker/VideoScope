...
- **Текущее доказанное состояние:** Фаза 0 открыта. Чистый `7054f13` на M4 Pro
  прошёл 2 527 backend-тестов, аттестацию шести окружений и принудительный rollback.
  Полный диагностический smoke завершил восемь шагов, пять продуктовых профилей
  и export; отдельная шкала этапов/RSS завершена и привязана к точным raw bytes.
  Resource gate не пройден: 176 страниц swap-in, 0 swap-out и Metal recovery,
  OOM не наблюдался. Отдельный валидный 120-секундный контроль, не запускавший
  inference или workers в своём дереве процессов, зарегистрировал 4 страницы swap-in (65 536 байт); это не устанавливает
  причину событий smoke и не разрешает вычитать фоновые значения. Первый запуск
  этого контроля невалиден из-за ошибки cleanup в частном driver; отказ сохранён,
  исправление проверено тремя тестами с fake providers. Точные receipts сохранены
  в [новом контрольном отчёте](benchmarks/phase0/negative-controls/7054f13/README.md).
  Пять benchmark-профилей и десять прямых verifier cases теперь выполнены на том
  же `7054f13`: 3 совпадения, 7 модельных промахов, 0 инфраструктурных ошибок;
  workers очищены. Все validators, кроме smoke, прошли. Полный collector вернул
  exit 3 и не создал bundle из-за тех же 176 страниц swap-in. Доказательства
  прежнего `1b11a54` сохранены отдельно и не смешиваются с новым SHA.
  Следующий ограниченный эксперимент — неизменённый обычный full smoke после
  предоставленной владельцем свежей macOS-сессии. На 6 сентября система всё ещё
  загружена 18 августа; свежая сессия не гарантирует нулевой swap.
  Порог не меняется, вычитания фона нет; обучение и следующие фазы не начаты.
...
### Первый следующий инкремент

Не начинать с загрузки X-CLIP или QLoRA. Следующая задача должна закрыть **Фазу 0 и
вертикальный срез Фазы 1**:

1. сделать текущий verifier/product benchmark полностью воспроизводимым из чистого
   checkout и завершить аттестацию measurement/runtime;
2. определить JSON schemas для `SourceManifest`, `Example`, `SplitManifest`,
   `TrainingRun` и `ModelRelease`;
3. реализовать validator и deterministic split/seal на tiny synthetic fixtures;
4. провести read-only аудит BARD/E-BARD и записать rights/data-quality report;
5. импортировать только разрешённый минимальный набор и собрать coverage plan до
   фактического обучения.

**Готово**, когда команда проверки отвергает leakage, неизвестное происхождение и
изменённый sealed snapshot; benchmark выдаёт повторяемый baseline; ни одно
пользовательское видео и ни один внешний dataset ещё не были тихо отправлены в
облако. После этого первый training experiment — frozen SigLIP2 feature control, а
не большая video model.
