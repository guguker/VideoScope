...
> Обновление 2026-08-23: решение после локального A/B, включая фактический OOM
> 27B и baseline в exploratory runtime 0.6.15, зафиксировано в
> [локальной модельной стратегии](local-model-strategy.md). Этот документ ниже
> сохраняет подробную историю предметного basketball-контура.
...
Один общий similarity больше не считается доказательством события. Тип броска
задаёт временной кандидат из последовательности наблюдений вроде «выпуск → мяч у
кольца → реакция». Qwen независимо подтверждает только наблюдаемый факт
`ball_through_hoop`; её предположение о дуге не подтверждает и не отменяет тип
события. Геометрия площадки или изменение счёта могут стать отдельным
подтверждающим сигналом. Без достаточного согласования результат остаётся
кандидатом.
...
В историческом storyboard-прогоне `Qwen3-VL-2B` прочитала номер 15 и увидела мяч в
кольце, но нестабильно определяла положение игрока относительно трёхочковой дуги.
Новый native-video A/B отдельно показал ошибку на этом же попадании. Поэтому 2B не
используется как источник окончательного решения.
...
- для размеченных made-shot кандидатов Qwen получает короткий native MP4; для
  остальных action-запросов используется раскадровка до 12 кадров;
...
- оценка Qwen положения относительно дуги не подтверждает и не veto'ит тип броска;
