...
with (OUTPUT/'no-ml-control.json').open('x') as out:
    ...
    try:
        host.start();trace.record_host_sampler_epoch(host.started_monotonic_ns);rss.start()
        ...
        trace.record_event('idle.end');process=rss.finish();resources=host.finish()
    finally:
        rss.close();host.close()
