...
2. Оставьте worker запущенным, но остановите основной VideoScope API.
3. Выполните `make index-lighthouse`; каждое видео получит новое поколение.
4. Снова запустите API и проверьте поиск и evaluation до удаления старого cache.

Команда использует тот же exclusive lock каталога `data/`, что и API, и
fail-fast при активном владельце. Не удаляйте `.videoscope-runtime.lock`:
наличие файла нормально, а конфликт означает, что процесс ещё держит `flock`.
